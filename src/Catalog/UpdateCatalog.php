...
class UpdateCatalog
{
    ...
    public function updateProduct($product)
    {
        ...
        try {
            ...
            $id_category_default = (int) self::getIdCategoryFromName(\Configuration::get('MPAPITYRES_CATEGORY_NAME'));
            ...
            if (\Validate::isLoadedObject($prestashopProduct)) {
                ...
                if (!$id_cover || self::FORCE_UPLOAD_IMAGES) {
                    $imageURL = self::createTyreUrl($product['idT24']);
                    self::addProductImageStatic($prestashopProduct->id, $imageURL);

                    $imageURL = str_replace('-%s-%s', '', $product['tyreLabel']['imageURL']);
                    self::addProductImageStatic($prestashopProduct->id, $imageURL);
                }
            ...
            if (!empty($product['imageURL'])) {
                $imageURL = self::createTyreUrl($product['idT24']);
                self::addProductImageStatic($id_product, $imageURL);
            }
