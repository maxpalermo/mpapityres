...
class UpdateCatalog extends \MpSoft\MpApiTyres\Helpers\DependencyHelper
{
    protected $languages;
    ...
    public function updateProduct($product)
    {
        ...
        try {
            ...
            if (\Validate::isLoadedObject($prestashopProduct)) {
                ...
                if (!$id_cover) {
                    $imageURL = str_replace('-%s-%s', '-w300-h300', $product['imageURL']);
                    $this->addProductImage($prestashopProduct->id, $imageURL);

                    $imageURL = str_replace('-%s-%s', '-w300-h300', $product['tyreLabel']['imageURL']);
                    $this->addProductImage($prestashopProduct->id, $imageURL);
                }

                // Attivo il prodotto
                $prestashopProduct->active = true;
                $prestashopProduct->update();
            ...
            // Categorie
            $id_category_default = (int) ($product['id_category_default'] ?? \Configuration::get('PS_HOME_CATEGORY'));
            ...
            if (!empty($product['imageURL'])) {
                $imageURL = str_replace('-%s', '', $product['imageURL']);
                $this->addProductImage($id_product, $imageURL);
            }

            // Gestione dell'immagine del label
            if (!empty($product['tyreLabel']['imageURL'])) {
                $imageURL = str_replace('-%s', '', $product['tyreLabel']['imageURL']);
                $this->addProductImage($id_product, $imageURL);
            }
     ...
     */
    protected function addProductImage($id_product, $image_url)
    {
        ...
        try {
            ...
            if ($image_downloaded) {
                ...
                \ImageManager::resize($temp_name, $image_path . '.jpg');
