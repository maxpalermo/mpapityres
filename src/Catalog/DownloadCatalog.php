...
class DownloadCatalog
{
    ...
    {
        \Configuration::updateValue(Constants::MPAPITYRES_CRON_DOWNLOAD_OFFSET, $this->offset);
        \Configuration::updateValue(Constants::MPAPITYRES_CRON_DOWNLOAD_UPDATED, $this->updated);
        \Configuration::updateValue(Constants::MPAPITYRES_CRON_DOWNLOAD_STATUS, $this->status);
        \Configuration::updateValue(Constants::MPAPITYRES_CRON_DOWNLOAD_UPDATED_DATE, $this->updated_date);
    }

    protected function readStatus()
    {
        $this->offset = (int) \Configuration::get(Constants::MPAPITYRES_CRON_DOWNLOAD_OFFSET) ?: 0;
        $this->updated = (int) \Configuration::get(Constants::MPAPITYRES_CRON_DOWNLOAD_UPDATED) ?: 0;
        $this->status = \Configuration::get(Constants::MPAPITYRES_CRON_DOWNLOAD_STATUS) ?: "FETCHING";
        $this->updated_date = \Configuration::get(Constants::MPAPITYRES_CRON_DOWNLOAD_UPDATED_DATE) ?: false;
        $this->min_stock = \Configuration::get(Constants::MPAPITYRES_MIN_STOCK) ?: 4;
    }
    ...
    public function run()
    {
        ...
        if ($this->offset == 0) {
            ...
            $pfx = _DB_PREFIX_;
            $query = "truncate table {$pfx}product_tyre";
